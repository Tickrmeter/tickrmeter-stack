...
import NativeDevicePageModel from "../../models/devicepage-native";

import { Mapping, humbl3MappingJsonData } from "../page-mode/interfaces";
...
const dbOperation = async (dbQuery: any, type: DBOpType) => {
  const DevicePagesModel = new NativeDevicePageModel();
...
const getMappingJson = (data: humbl3MappingJsonData) => {
  ...
  //const _diff = data?.diff ? Math.abs(data.diff)?.toFixed(2) + "%" : "";
  ...
  //if (mappingJson.Type.toString().includes("Artist Streams")) {
