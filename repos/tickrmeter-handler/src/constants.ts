...
if (env === "dev") {
  const apiAppPath = path.join(__dirname, "../..", "epaper-stock-price");
  fontDirectory = path.join(apiAppPath, "src/fonts");
  pageDirectory = path.join(apiAppPath, "device-pages");
  templateDirectory = path.join(apiAppPath, "src/svg-templates");
}
...
//console.log("apiAppPath", apiAppPath);
...
export const SYMBOL_TYPE_OBJ: ISYMBOL_TYPE_OBJ[] = [
  ...
  { id: SYMBOL_TYPE.CUSTOMAPI, name: "Custom API", value: "custom-api", mode: "text" },
  { id: SYMBOL_TYPE.MUSIC_CREATORS, name: "Music & Creators", value: "music-creators", mode: "page" },
  { id: SYMBOL_TYPE.FXON, name: "FXON", value: "fxon", mode: "text" },
];
