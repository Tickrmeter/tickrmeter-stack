...
export const GetPolygonStockQuoteFromWS: IGetQuoteFromAPI = async (params) => {
  try {
    ...
    console.log("Polygon Stock: Fetching stock quote for", symbol, "from Redis", data);
...
const getStockQuoteFromAPI = async (symbol: string) => {
  ...
  try {
    ...
    console.log("Polygon Stock: Fetching stock quote for", symbol, "from API", url);

    let response: any = await http.getAPI(url, APISOURCES.POLYGON, true);

    if (response.status === "IEX") response.status = "OK"; // IEX is used for stocks, so we treat it as OK
