...
export class QuoteChannel {
  ...
  constructor(quoteSessionBridge: QuoteSession, pairGroups: string[] | KeyTo<string[]>, fields: Array<Field> = []) {
  ...
  public addSymbolToPair(symbol: string): void {
    if (this.pairList.has(symbol)) return;

    this.pairList.add(symbol);
    console.log("pairList", this.pairList.size);
    this.$adapter.removePairs(this.pairList);
    this.$adapter.addPairs(symbol);
  }

  public removeSymbolFromPair(symbol: string): void {
    if (!this.pairList.has(symbol)) return;

    this.pairList.delete(symbol);
    this.$adapter.removePairs(symbol);
  ...
  public listen(
    callback: (data: KeyMayTo<KeyTo<QuotedData>>, flags: { firstLoad: boolean; reload: boolean }) => void
  ): void {
