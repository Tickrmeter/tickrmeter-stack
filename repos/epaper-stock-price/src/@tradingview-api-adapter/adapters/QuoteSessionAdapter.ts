import { exchangeMarketMap } from "../../constants";
...
export class QuoteSessionAdapter extends EventEmitter {
  ...
  constructor(private readonly $bridge: QuoteSession, private readonly $isMultiChannel = false) {
    super();
  ...
  private handleSessionListener(sessionData: { params: Array<ParameterFields>; uploaded?: boolean }): void {
    ...
    sessionData.params.forEach((param) => {
      switch (param.s) {
        case "error":
          console.error("Error", param.n, param.errmsg); //here I will need to add an error throw event
          break;
        case "ok":
          if (this.$isMultiChannel) {
            const separatedPair = param.n.split(":");

            const [exchange, symbol] = separatedPair;

            const objectKey = `${exchange}:${symbol}`;

            console.log({ param });
            const qObj = { [objectKey]: param.v };

            Object.assign(quotedData, qObj);
