...
static IRAM_ATTR const char TAG[] = "xmc_sr";
...
extern char loglbuf[2048];

void loglocal(const char *format, ...)
{
    va_list args;
    va_start(args, format);
    char llog[512];
    vsnprintf(llog, sizeof(llog), format, args);
    va_end(args);
    printf("%s\n", llog);
    strcat(llog, "|");
    strncat(loglbuf, llog, sizeof(llog));
    //append to 
}
...
bool IRAM_ATTR xmc_lock_sr(uint32_t good_value, bool permanent)
{
    bool skip = false;
    ...
    // loglocal("Disable interrupts, caches and other CPU.");
    spi_flash_disable_interrupts_caches_and_other_cpu();
    // loglocal("Wait for flash idle.");
    esp_rom_spiflash_wait_idle(&g_rom_flashchip);
    // loglocal("Read SR before.");
    ...
    // loglocal("SR1 & SR2 before: %02X%02X (%04"PRIX32").", sr2_before, sr1_before, sr12_before);
   ...
   if (sr12_before & SR_SRP1_BIT) {
    //    loglocal("SRP1 already set (%02X%02X), skip SR lock.", sr2_before, sr1_before);
       skip = true;
   }
   ...
   if (!skip && sr12_set != sr12_before) {
    ...
    spi_flash_enable_interrupts_caches_and_other_cpu();
    if (skip) {
        loglocal("SRP1 already set (%02X%02X%02X), skip SR lock.", sr3_before, sr2_before, sr1_before);
        return false;
    } else {
        if (permanent) {
            loglocal("Try lock SR permanently.");
        } else {
            loglocal("Try lock SR until power cycle.");
        }
        loglocal("SR3 update: %02X -> %02X (%02X).", sr3_before, sr3_after, sr3_set);
        loglocal("SR1 & SR2 update: %02X%02X -> %02X%02X (%04"PRIX32").", sr2_before, sr1_before, sr2_after, sr1_after, sr12_set);
        loglocal( "SR1 & SR2 update: %02X%02X -> %02X%02X (%04"PRIX32").", sr2_before, sr1_before, sr2_after, sr1_after, sr12_set);
    }
    return true;
}

int IRAM_ATTR xmc_check_lock_sr(bool permanent)
{
    uint32_t flash_id;
    uint8_t sfdp_06;
    uint8_t sfdp_f4;
    //These IDs are used to identify XMC flash chips with SR lock issue.
    loglocal("SR lock check start.");
    xmc_read_flash_id(&flash_id, &sfdp_06, &sfdp_f4);
    loglocal("Flash ID: %06"PRIX32", SFDP 0x06: %02X, SFDP 0xF4: %02X.", flash_id, sfdp_06, sfdp_f4);

    sprintf(chipID, "%06"PRIX32, flash_id);

    if (!IS_XMC(flash_id)) {
        loglocal("non-xmc (%06"PRIX32"). SR lock skipped.", flash_id);
        return 1;
    } else if (!XMC_NEEDS_SR_LOCK(sfdp_06, sfdp_f4)) {
        loglocal("ver not match (%02X, %02X), SR lock skipped.", sfdp_06, sfdp_f4);
        return 2;
    }
    ...
    if (flash_id == 0x204016) {
        ...
        loglocal("Unsupported XMC model: %06"PRIX32, flash_id);
        return 3;
    }
    loglocal("Calling SR lock with good value: %06"PRIX32, good_value);
    if (!xmc_lock_sr(good_value, permanent))
        return 6;
    return 0;
}
