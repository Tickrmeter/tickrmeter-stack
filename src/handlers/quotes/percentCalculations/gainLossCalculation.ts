...
const calculateStockProfit = ({ purchasePrice, noOfStocks, isShortSell, currentPrice }) => {
  ...
  if (isNaN(parsedPurchasePrice) || isNaN(parsedNoOfStocks) || parsedPurchasePrice < 0 || parsedNoOfStocks < 0) {
    console.log("----Invalid input for calculateStockProfit----");
    console.log({ purchasePrice, noOfStocks, isShortSell, currentPrice });
    console.log({ parsedPurchasePrice, parsedNoOfStocks });
    return null;
  }
