...
export const getLEDBrightness = ({ tz, startTime, endTime, brightness, macAddress }): number => {
  ...
  return isWithinNightModeRange(currentTime, nightModeStart, nightModeEnd, tz, macAddress) ? 0 : brightness;
};

export const isWithinNightModeRange = (
  ...
  try {
    ...
    console.error("Error in isWithinNightModeRange: ", macAddress, error);
    return false;
  }
};
